...
export const StudentHomeScreen = () => {
  const { theme, language, currentUser, invoices, announcements, requests, navigate } = useApp();
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.summaryContainer}>
          ...
          >
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={[styles.summaryTitle, { color: colors.textSecondary }]}>{t('pendingInvoices', language)}</Text>
              <Text style={[styles.summaryVal, { color: unpaidCount > 0 ? colors.danger : colors.success }]}>
                {t('invoicesCount', language, { count: unpaidCount })}
              </Text>
            </View>
          </Card>
          ...
          >
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={[styles.summaryTitle, { color: colors.textSecondary }]}>{t('repairRequests', language)}</Text>
              <Text style={[styles.summaryVal, { color: colors.text }]}>
                {t('processingCount', language, { count: activeRequests })}
              </Text>
            </View>
          </Card>
        </View>
        ...
        {/* Quick actions grid */}
        <Text style={[styles.sectionTitle, { color: colors.text }]}>{t('dormServices', language)}</Text>
        <View style={styles.actionGrid}>
          ...
          >
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={[styles.actionLabel, { color: colors.text }]}>{t('roomAndRoommates', language)}</Text>
            </View>
          </Card>
          ...
          >
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={[styles.actionLabel, { color: colors.text }]}>{t('payInvoice', language)}</Text>
            </View>
          </Card>
          ...
          >
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={[styles.actionLabel, { color: colors.text }]}>{t('reportIncident', language)}</Text>
            </View>
          </Card>
          ...
          >
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={[styles.actionLabel, { color: colors.text }]}>{t('settings', language)}</Text>
            </View>
          </Card>
        </View>
        ...
        <View style={styles.announcementHeader}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>{t('announcementsFromBQL', language)}</Text>
          <Card style={{ paddingHorizontal: 10, paddingVertical: 4, marginVertical: 0 }} onPress={() => navigate('Notifications')}>
            <Text style={[styles.seeAllText, { color: colors.primary }]}>{t('seeAll', language)}</Text>
          </Card>
        </View>
            ...
            <View style={styles.annTitleRow}>
              {ann.priority === 'important' && (
                <View style={styles.importantBadge}>
                  <Text style={styles.importantText}>{t('important', language)}</Text>
                </View>
              )}
        ...
        <Card style={[styles.infoBanner, { backgroundColor: `${colors.primary}0D`, borderColor: `${colors.primary}30` }]}>
          ...
          <View style={styles.infoContent}>
            <Text style={[styles.infoTitle, { color: colors.primary }]}>{t('dormRules', language)}</Text>
            <Text style={[styles.infoText, { color: colors.text }]}>
              {t('curfewNotice', language)}
            </Text>
          </View>
        </Card>
      </ScrollView>
